              ...
              <motion.div className="relative max-w-5xl max-h-[90vh] w-full flex items-center justify-center" onClick={(e) => e.stopPropagation()} layoutId={`card-image-container-${selectedId}`}>
                <AnimatePresence mode="wait">
                  <motion.div key={selectedId} className="w-full h-full flex items-center justify-center">
                    {selectedItem.mediaType === 'image' ? (<img src={selectedItem.mediaUrl} alt={selectedItem.title} className="w-auto h-auto object-contain max-h-[90vh] rounded-lg shadow-2xl"/>) : (<video src={selectedItem.mediaUrl} className="w-auto h-auto object-contain max-h-[90vh] rounded-lg shadow-2xl" controls autoPlay/>)}
                  </motion.div>
                </AnimatePresence>
              </motion.div>
