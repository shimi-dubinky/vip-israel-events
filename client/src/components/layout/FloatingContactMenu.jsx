...
const FloatingContactMenu = () => {
  ...
  // --- פרטי יצירת קשר ---
  const phoneNumber = "972587554578"; // החלף במספר הרצוי
  const email = "7686266@gmail.com"; // החלף באימייל הרצוי
  ...
  const positions = [
    { y: -70, x: 0 }, // למעלה
    { y: -50, x: -50 }, // למעלה-שמאלה
    { y: 0, x: -70 }, // שמאלה
  ];

  return (
    // קונטיינר שמגיב לריחוף בדסקטופ
    <div 
      className="fixed bottom-6 right-6 z-50"
      onMouseEnter={() => setIsOpen(true)}
      onMouseLeave={() => setIsOpen(false)}
    >
      {/* תפריט האייקונים הקטנים */}
      <AnimatePresence>
        {isOpen && (
          <motion.div>
            {links.map((link, i) => (
